...
// --- SHOP & PERSISTENCE ---
const STORAGE_KEY_GOLD = 'ratato_total_gold';
const STORAGE_KEY_POWERUPS = 'ratato_powerups';

let totalGold = parseInt(localStorage.getItem(STORAGE_KEY_GOLD) || '0', 10);
let purchasedPowerUps = JSON.parse(localStorage.getItem(STORAGE_KEY_POWERUPS) || '{}');

function saveGame() {
    localStorage.setItem(STORAGE_KEY_GOLD, totalGold);
    localStorage.setItem(STORAGE_KEY_POWERUPS, JSON.stringify(purchasedPowerUps));
}

const POWER_UP_STATS = {
    might: { name: "Might", description: "Increases damage dealt by 5%.", cost: 200, maxRank: 5, stat: 'damageMultiplier', amount: 0.05 },
    armor: { name: "Armor", description: "Reduces incoming damage by 1.", cost: 600, maxRank: 3, stat: 'armor', amount: 1 },
    maxHealth: { name: "Max Health", description: "Increases max health by 10%.", cost: 200, maxRank: 5, stat: 'maxHp', amount: 0.1 }, // Multiplier logic needed or flat? VS is %
    recovery: { name: "Recovery", description: "Recovers 0.1 HP per second.", cost: 200, maxRank: 5, stat: 'hpRegen', amount: 0.1 },
    cooldown: { name: "Cooldown", description: "Reduces weapon cooldowns by 2.5%.", cost: 900, maxRank: 2, stat: 'fireRateReduction', amount: 0.025 },
    area: { name: "Area", description: "Increases area of effect by 5%.", cost: 300, maxRank: 2, stat: 'attackRangeMultiplier', amount: 0.05 },
    speed: { name: "Speed", description: "Increases projectile speed by 10%.", cost: 300, maxRank: 2, stat: 'projectileSpeed', amount: 0.1 }, // Need to implement prop
    duration: { name: "Duration", description: "Increases effect duration by 15%.", cost: 300, maxRank: 2, stat: 'durationMultiplier', amount: 0.15 },
    amount: { name: "Amount", description: "Fires +1 projectile (Max 1).", cost: 5000, maxRank: 1, stat: 'projectileCountBonus', amount: 1 },
    moveSpeed: { name: "Move Speed", description: "Increases movement speed by 5%.", cost: 300, maxRank: 2, stat: 'baseSpeed', amount: 0.05 }, // Multiplier logic needed
    magnet: { name: "Magnet", description: "Increases item pickup range by 25%.", cost: 300, maxRank: 2, stat: 'magnetRange', amount: 0.25 }, // Multiplier logic needed
    luck: { name: "Luck", description: "Increases luck by 10%.", cost: 600, maxRank: 3, stat: 'luck', amount: 0.1 },
    greed: { name: "Greed", description: "Gains 10% more coins.", cost: 200, maxRank: 5, stat: 'greed', amount: 0.1 },
    growth: { name: "Growth", description: "Gains 3% more experience.", cost: 900, maxRank: 5, stat: 'xpMultiplier', amount: 0.03 },
    skip: { name: "Skip", description: "+1 Skip per rank.", cost: 100, maxRank: 2, stat: 'skips', amount: 1 },
    reroll: { name: "Reroll", description: "+1 Reroll per rank.", cost: 1000, maxRank: 2, stat: 'rerolls', amount: 1 },
    revival: { name: "Revival", description: "+1 Revival (Max 1).", cost: 10000, maxRank: 1, stat: 'revivals', amount: 1 },
    curse: { name: "Curse", description: "Increases enemy speed, health, spawn rate by 10%.", cost: 1666, maxRank: 5, stat: 'curse', amount: 0.1 }
};
...
const CHARACTERS = {
    // 1. Rattonio (Antonio)
    rattonio: {
        name: "Rattonio Belpaese",
        description: "Gains 10% more damage every 10 levels (max +50%). High HP.",
        initialStats: {
            maxHp: 120, baseSpeed: 3.5, damageMultiplier: 1.1, armor: 1, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.0, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.0, magnetRange: 50, xpMultiplier: 1.0, revivals: 0, curse: 1.0, greed: 1.0
        },
        ratColors: { main: '#8B4513', light: '#DEB887', detail: '#5D4037' }, // Brown
        starterWeapon: 'whip',
        accessories: [{ type: 'bandana', color: '#ff0000' }]
    },
    // 2. Rammelda (Imelda)
    rammelda: {
        name: "Rammelda Belpaese",
        description: "Gains 10% more experience every 5 levels (max +30%).",
        initialStats: {
            maxHp: 100, baseSpeed: 4.0, damageMultiplier: 1.0, armor: 0, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.0, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.0, magnetRange: 60, xpMultiplier: 1.1, revivals: 0, curse: 1.0, greed: 1.0
        },
        ratColors: { main: '#A0522D', light: '#F4A460', detail: '#800080' }, // Sienna w/ Purple
        starterWeapon: 'magicWand',
        accessories: [{ type: 'ribbon', color: '#800080' }]
    },
    // 3. Ratsqualina (Pasqualina)
    ratsqualina: {
        name: "Ratsqualina Belpaese",
        description: "Projectiles get +10% speed every 5 levels (max +30%).",
        initialStats: {
            maxHp: 100, baseSpeed: 3.8, damageMultiplier: 1.0, armor: 0, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.0, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 1,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.0, magnetRange: 50, xpMultiplier: 1.0, revivals: 0, curse: 1.0, greed: 1.0
        },
        ratColors: { main: '#CD853F', light: '#FFDAB9', detail: '#4682B4' }, // Peru w/ Blue
        starterWeapon: 'runetracer',
        accessories: [{ type: 'hat_wizard', color: '#4682B4' }]
    },
    // 4. Ratnaro (Gennaro)
    ratnaro: {
        name: "Ratnaro Belpaese",
        description: "Permanent +1 projectile count to all weapons.",
        initialStats: {
            maxHp: 110, baseSpeed: 4.0, damageMultiplier: 1.0, armor: 0, evasion: 0,
            projectileCountBonus: 1, fireRateReduction: 1.0, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.0, magnetRange: 50, xpMultiplier: 1.0, revivals: 0, curse: 1.0, greed: 1.0
        },
        ratColors: { main: '#696969', light: '#D3D3D3', detail: '#000000' }, // Grey
        starterWeapon: 'knife',
        accessories: [{ type: 'bandana', color: '#333' }]
    },
    // 5. Raca (Arca)
    raca: {
        name: "Raca Ladonna",
        description: "Weapon cooldown reduces by 5% every 10 levels (max -15%).",
        initialStats: {
            maxHp: 100, baseSpeed: 3.8, damageMultiplier: 1.0, armor: 0, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.1, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.0, magnetRange: 50, xpMultiplier: 1.0, revivals: 0, curse: 1.0, greed: 1.0
        },
        ratColors: { main: '#B22222', light: '#FA8072', detail: '#FF4500' }, // Fire Red
        starterWeapon: 'fireWand',
        accessories: [{ type: 'beard', color: '#FF4500' }]
    },
    // 6. Rorta (Porta)
    rorta: {
        name: "Rorta Ladonna",
        ...
        initialStats: {
            maxHp: 120, baseSpeed: 3.5, damageMultiplier: 1.0, armor: 0, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.0, attackRangeMultiplier: 1.3, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.0, magnetRange: 50, xpMultiplier: 1.0, revivals: 0, curse: 1.0, greed: 1.0
        },
        ratColors: { main: '#4B0082', light: '#9370DB', detail: '#8A2BE2' }, // Indigo
        starterWeapon: 'lightningRing',
        accessories: [{ type: 'hood', color: '#4B0082' }]
    },
    // 7. Rama (Lama)
    rama: {
        name: "Rama Ladonna",
        ...
        initialStats: {
            maxHp: 110, baseSpeed: 4.2, damageMultiplier: 1.1, armor: 0, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.0, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.0, magnetRange: 50, xpMultiplier: 1.0, revivals: 0, curse: 1.1, greed: 1.0
        },
        ratColors: { main: '#8B0000', light: '#CD5C5C', detail: '#000' }, // Dark Red
        starterWeapon: 'axe',
        accessories: [{ type: 'beard', color: '#000' }]
    },
    // 8. Poe Ratcho (Poe)
    poe: {
        name: "Poe Ratcho",
        description: "Permanent +25% Pickup Range. Starts with -30 Max Health.",
        initialStats: {
            maxHp: 70, baseSpeed: 3.5, damageMultiplier: 1.0, armor: 0, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.0, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.0, magnetRange: 100, xpMultiplier: 1.0, revivals: 0, curse: 1.0, greed: 1.0
        },
        ratColors: { main: '#F5DEB3', light: '#FFF8DC', detail: '#D2691E' }, // Old/Beige
        starterWeapon: 'garlic',
        accessories: [{ type: 'beard', color: '#fff' }, { type: 'glasses', color: '#000' }]
    },
    // 9. Suor Ratrica (Clerici)
    ratrica: {
        name: "Suor Ratrica",
        description: "Permanent +0.5 HP Recovery and +50 Max Health.",
        initialStats: {
            maxHp: 150, baseSpeed: 3.5, damageMultiplier: 1.0, armor: 0, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.0, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.0, magnetRange: 50, xpMultiplier: 1.0, revivals: 0, curse: 1.0, greed: 1.0,
            hpRegen: 0.5
        },
        ratColors: { main: '#4682B4', light: '#87CEEB', detail: '#000080' }, // Blue nun
        starterWeapon: 'santaWater',
        accessories: [{ type: 'hood', color: '#000080' }]
    },
    // 10. Ratommario (Dommario)
    ratommario: {
        name: "Ratommario",
        description: "Permanent +40% Duration and Speed. -40% MoveSpeed.",
        initialStats: {
            maxHp: 100, baseSpeed: 2.5, damageMultiplier: 1.0, armor: 0, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.4, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.0, magnetRange: 50, xpMultiplier: 1.0, revivals: 0, curse: 1.0, greed: 1.0
        },
        ratColors: { main: '#DC143C', light: '#FF6347', detail: '#FFD700' }, // Cardinal Red
        starterWeapon: 'kingBible',
        accessories: [{ type: 'hat_wizard', color: '#DC143C' }, { type: 'beard', color: '#ccc' }]
    },
    // 11. Ratochi (Krochi)
    ratochi: {
        name: "Ratochi Freetto",
        description: "Starts with 1 Revival. Gains +1 Revival at Level 33.",
        initialStats: {
            maxHp: 100, baseSpeed: 5.0, damageMultiplier: 1.0, armor: 0, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.0, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.0, magnetRange: 50, xpMultiplier: 1.0, revivals: 1, curse: 1.0, greed: 1.0
        },
        ratColors: { main: '#800000', light: '#A52A2A', detail: '#000' }, // Demon red
        starterWeapon: 'cross',
        accessories: [{ type: 'mohawk', color: '#f00' }]
    },
    // 12. Ratstine (Christine)
    ratstine: {
        name: "Ratstine Davain",
        description: "Starts with 1 level in Pentagram. -50% cooldown reduction.",
        initialStats: {
            maxHp: 50, baseSpeed: 4.5, damageMultiplier: 0.8, armor: 0, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.5, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.3, magnetRange: 50, xpMultiplier: 1.2, revivals: 0, curse: 1.0, greed: 1.0
        },
        ratColors: { main: '#F0FFFF', light: '#FFFFFF', detail: '#ADD8E6' }, // White/Ice
        starterWeapon: 'pentagram',
        accessories: [{ type: 'ribbon', color: '#ADD8E6' }]
    },
    // 13. Ratnala (Pugnala)
    ratnala: {
        name: "Ratnala Provola",
        description: "Gains +1% Might every level (no cap).",
        initialStats: {
            maxHp: 100, baseSpeed: 4.0, damageMultiplier: 1.0, armor: 0, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.0, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
        ...
        ratColors: { main: '#C0C0C0', light: '#DCDCDC', detail: '#FF00FF' }, // Silver w/ Pink
        starterWeapon: 'guns',
        accessories: [{ type: 'hat_wizard', color: '#FF00FF' }]
    },
    // 14. Rattaccio (Mortaccio)
    rattaccio: {
        name: "Rattaccio",
        description: "Gains +1 Projectile every 20 levels (max +3).",
        initialStats: {
            maxHp: 100, baseSpeed: 3.5, damageMultiplier: 1.0, armor: 1, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.0, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.0, magnetRange: 50, xpMultiplier: 1.0, revivals: 0, curse: 1.0, greed: 1.0
        },
        ratColors: { main: '#F5F5DC', light: '#FFFFF0', detail: '#8B4513' }, // Bone
        starterWeapon: 'bone',
        accessories: []
    },
    // 15. Ravallo (Cavallo)
    ravallo: {
        ...
        initialStats: {
            ...
            projectileCountBonus: 0, fireRateReduction: 1.0, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.1, magnetRange: 50, xpMultiplier: 1.0, revivals: 0, curse: 1.0, greed: 1.0
        },
        ratColors: { main: '#A52A2A', light: '#D2691E', detail: '#000' }, // Brown/Panda?
        starterWeapon: 'cherryBomb',
        accessories: [{ type: 'glasses', color: '#000' }]
    },
    // 16. Ramba (Bianca Ramba)
    ramba: {
        name: "Bianca Ramba",
        description: "Gains +1 Projectile every 20 levels (max +3).",
        initialStats: {
        ...
        ratColors: { main: '#FFFFE0', light: '#FFFFF0', detail: '#808080' }, // Old lady
        starterWeapon: 'carrello',
        accessories: []
    },
    // 17. Rat'Sole (O'Sole Meeo)
    ratsole: {
        name: "Rat'Sole Meeo",
        description: "Gains +1 Projectile every 20 levels (max +3).",
        initialStats: {
            maxHp: 100, baseSpeed: 3.5, damageMultiplier: 1.0, armor: 0, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.0, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.0, magnetRange: 50, xpMultiplier: 1.0, revivals: 0, curse: 1.0, greed: 1.0
        },
        ratColors: { main: '#FFF', light: '#EEE', detail: '#FFD700' }, // Dog
        starterWeapon: 'celestialDusting',
        accessories: []
    },
    // 18. Rambojoe (Ambrojoe)
    rambojoe: {
        name: "Sir Rambojoe",
        description: "Gains +1 Projectile every 20 levels (max +3).",
        initialStats: {
            ...
            projectileCountBonus: 0, fireRateReduction: 1.0, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.2, magnetRange: 50, xpMultiplier: 1.0, revivals: 0, curse: 1.0, greed: 1.0
        },
        ratColors: { main: '#000', light: '#333', detail: '#fff' }, // Tuxedo
        starterWeapon: 'laRobba',
        accessories: [{ type: 'hat_wizard', color: '#000' }]
    },
    // 19. Rallo (Gallo)
    rallo: {
        name: "Iguana Rallo",
        description: "Permanent +15% Duration. -15% Cooldown. +3 Reroll/Skip.",
        initialStats: {
            maxHp: 100, baseSpeed: 3.5, damageMultiplier: 1.0, armor: 0, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.15, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.0, magnetRange: 50, xpMultiplier: 1.0, revivals: 0, curse: 1.0, greed: 1.0,
            rerolls: 3, skips: 3
        },
        ratColors: { main: '#FFA500', light: '#FFD700', detail: '#FF4500' }, // Orange
        starterWeapon: 'clockLancet',
        accessories: []
    },
    // 20. Rivano (Divano)
    rivano: {
        name: "Rivano Thelma",
        description: "Permanent +5 Armor. +3 Reroll/Banish.",
        initialStats: {
            maxHp: 100, baseSpeed: 3.5, damageMultiplier: 1.0, armor: 5, evasion: 0,
            projectileCountBonus: 0, fireRateReduction: 1.0, attackRangeMultiplier: 1.0, shield: 0, bounceCount: 0,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.0,
            luck: 1.0, magnetRange: 50, xpMultiplier: 1.3, revivals: 0, curse: 1.0, greed: 1.0,
            rerolls: 3, banishes: 3
        },
        ratColors: { main: '#800080', light: '#9370DB', detail: '#fff' }, // Purple
        starterWeapon: 'laurel',
        accessories: []
    },
    // 21. Queen Rat (Sigma)
    queenrat: {
        name: "Queen Rat Sigma",
        description: "She owns everything. Extreme stats.",
        initialStats: {
            maxHp: 333, baseSpeed: 5.0, damageMultiplier: 1.5, armor: 3, evasion: 0.1,
            projectileCountBonus: 1, fireRateReduction: 1.2, attackRangeMultiplier: 1.2, shield: 0, bounceCount: 1,
            lifestealPercentage: 0, currency: 0, maxWeapons: 6, meleeRangeMultiplier: 1.2,
            luck: 1.5, magnetRange: 100, xpMultiplier: 1.5, revivals: 3, curse: 1.0, greed: 1.0,
            rerolls: 5, skips: 5, banishes: 5
        },
        ratColors: { main: '#FFD700', light: '#FFFACD', detail: '#DAA520' }, // Gold
        starterWeapon: 'victorySword',
        accessories: [{ type: 'hat_wizard', color: '#FFD700' }]
    }
};

// --- WEAPON HELPER ---
function createWeapon(key, name, emoji, description, type, requiredPassive, evolvesTo, iconColor, baseStats, levelUpgrades) {
    const levels = [];
    // Level 1: Base Stats
    let currentStats = { ...baseStats };
    levels.push({ ...currentStats });

    // Generate Levels 2-8 (i=0 is Level 2)
    for (let i = 0; i < 7; i++) {
        const upgrade = (levelUpgrades && i < levelUpgrades.length) ? levelUpgrades[i] : null;
        if (upgrade) {
            Object.keys(upgrade).forEach(k => {
                // If the key exists in base stats and is a number, treat as delta
                if (typeof currentStats[k] === 'number' && typeof upgrade[k] === 'number') {
                     currentStats[k] += upgrade[k];
                } else {
                     // Otherwise override (e.g. changing cooldown significantly or non-numeric)
                     currentStats[k] = upgrade[k];
                }
            });
        }
    ...
    return {
        key, name, emoji, description, type, requiredPassive, evolvesTo, iconColor, levels
    };
}
...
// --- WEAPON DEFINITIONS ---
const WEAPONS = {
    whip: createWeapon('whip', "Whip", "🐁", "Strikes horizontally.", 'directional-melee', 'emptyHeart', 'bloodyTear', '#b794f4',
        { damage: 10, amount: 1, cooldown: 1350, area: 1.0, speed: 0, duration: 200, knockback: 3, pierce: 99 },
        [{ damage: 5 }, { damage: 5 }, { damage: 5, area: 0.1 }, { damage: 5 }, { damage: 5, area: 0.1 }, { damage: 5 }, { damage: 5, area: 0.1 }]
    ),
    magicWand: createWeapon('magicWand', "Magic Wand", "🪄", "Fires at nearest enemy.", 'projectile-nearest', 'emptyTome', 'holyWand', '#f6ad55',
        { damage: 10, amount: 1, cooldown: 1000, area: 1, speed: 10, duration: 0, knockback: 2, pierce: 0 },
        [{ amount: 1 }, { cooldown: -100 }, { amount: 1 }, { damage: 10 }, { amount: 1 }, { pierce: 1 }, { damage: 10 }]
    ),
    knife: createWeapon('knife', "Knife", "🗡️", "Fires in facing direction.", 'projectile-facing', 'bracer', 'thousandEdge', '#718096',
        { damage: 7, amount: 1, cooldown: 300, area: 1, speed: 15, duration: 0, knockback: 1, pierce: 0 },
        [{ amount: 1 }, { amount: 1 }, { damage: 5 }, { amount: 1 }, { amount: 1, pierce: 1 }, { damage: 5 }, { amount: 1, pierce: 1 }]
    ),
    axe: createWeapon('axe', "Axe", "🪓", "Thrown in a high arc.", 'projectile-upward', 'candelabrador', 'deathSpiral', '#713e2f',
        { damage: 20, amount: 1, cooldown: 1500, area: 1.5, speed: 8, duration: 0, knockback: 5, pierce: 4 },
        [{ amount: 1 }, { damage: 10 }, { area: 0.2 }, { amount: 1 }, { damage: 10 }, { area: 0.2 }, { amount: 1 }]
    ),
    cross: createWeapon('cross', "Cross", "✝️", "Returns like a boomerang.", 'boomerang', 'clover', 'heavenSword', '#fff',
        { damage: 10, amount: 1, cooldown: 1000, area: 1, speed: 12, duration: 0, knockback: 5, pierce: 99 },
        [{ damage: 5 }, { area: 0.1 }, { damage: 5 }, { area: 0.1 }, { damage: 5 }, { area: 0.1 }, { damage: 10 }]
    ),
    kingBible: createWeapon('kingBible', "King Bible", "📖", "Orbits around the player.", 'orbit', 'spellbinder', 'unholyVespers', '#63b3ed',
        { damage: 10, amount: 1, cooldown: 3000, area: 1.0, speed: 5, duration: 3000, knockback: 2, pierce: 99 },
        [{ amount: 1 }, { duration: 500, speed: 1 }, { amount: 1, damage: 5 }, { duration: 500, speed: 1 }, { amount: 1, damage: 5 }, { duration: 500, speed: 1 }, { amount: 1, damage: 5 }]
    ),
    fireWand: createWeapon('fireWand', "Fire Wand", "🔥", "Fires at random enemy, high damage.", 'projectile-nearest', 'spinach', 'hellfire', '#e53e3e',
        { damage: 20, amount: 1, cooldown: 1200, area: 1, speed: 6, duration: 0, knockback: 2, pierce: 0 },
        [{ damage: 10 }, { damage: 10 }, { damage: 10 }, { damage: 10 }, { damage: 10 }, { damage: 10 }, { damage: 10 }]
    ),
    garlic: createWeapon('garlic', "Garlic", "🧄", "Damages nearby enemies.", 'aura', 'pummarola', 'soulEater', '#9ae6b4',
        { damage: 5, amount: 1, cooldown: 200, area: 1.0, speed: 0, duration: 0, knockback: 1, pierce: 99 },
        [{ area: 0.2 }, { damage: 2 }, { area: 0.2 }, { damage: 2 }, { area: 0.2 }, { damage: 2 }, { area: 0.2 }]
    ),
    santaWater: createWeapon('santaWater', "Santa Water", "💧", "Generates damaging zones.", 'zone-random', 'attractorb', 'laBorra', '#4299e1',
        { damage: 10, amount: 1, cooldown: 2000, area: 1.0, speed: 0, duration: 2000, knockback: 0, pierce: 99 },
        [{ amount: 1 }, { damage: 10 }, { duration: 500 }, { amount: 1 }, { damage: 10 }, { duration: 500 }, { area: 0.2 }]
    ),
    runetracer: createWeapon('runetracer', "Runetracer", "💎", "Pierces and bounces.", 'projectile-facing', 'armor', 'noFuture', '#ffffff',
        { damage: 10, amount: 1, cooldown: 1000, area: 1, speed: 15, duration: 2000, knockback: 1, pierce: 99, bounceCount: 3 },
        [{ speed: 3 }, { damage: 5 }, { duration: 500 }, { speed: 3 }, { damage: 5 }, { duration: 500 }, { amount: 1 }]
    ),
    lightningRing: createWeapon('lightningRing', "Lightning Ring", "⚡", "Strikes random enemies.", 'lightning', 'duplicator', 'thunderLoop', '#63b3ed',
        { damage: 15, amount: 2, cooldown: 1500, area: 1, speed: 0, duration: 0, knockback: 0, pierce: 0 },
        [{ amount: 1 }, { damage: 10 }, { amount: 1 }, { damage: 10 }, { amount: 1 }, { damage: 20 }, { amount: 1 }]
    ),
    pentagram: createWeapon('pentagram', "Pentagram", "🔯", "Erases enemies.", 'screen-wipe', 'crown', 'gorgeousMoon', '#b794f4',
        { damage: 9999, amount: 1, cooldown: 60000, area: 1, speed: 0, duration: 0, knockback: 0, pierce: 0 },
        [{ cooldown: -5000 }, { cooldown: -5000 }, { cooldown: -5000 }, { cooldown: -5000 }, { cooldown: -5000 }, { cooldown: -5000 }, { cooldown: -5000 }]
    ),
    guns: createWeapon('guns', "Phiera Der Tuphello", "🔫", "Fires diagonally.", 'projectile-facing', 'tiragisu', 'phieraggi', '#ff0000',
        { damage: 10, amount: 4, cooldown: 500, area: 1, speed: 15, duration: 0, knockback: 1, pierce: 1 },
        [{ amount: 1 }, { pierce: 1 }, { amount: 1 }, { damage: 5 }, { amount: 1 }, { pierce: 1 }, { damage: 5 }]
    ),
    cats: createWeapon('cats', "Gatti Amari", "🐈", "Cats wander and fight.", 'boomerang', 'stoneMask', 'viciousHunger', '#ffffff',
        { damage: 15, amount: 1, cooldown: 1000, area: 1, speed: 10, duration: 2000, knockback: 2, pierce: 99, bounceCount: 5 },
        [{ amount: 1 }, { damage: 5 }, { duration: 500 }, { amount: 1 }, { damage: 5 }, { duration: 500 }, { amount: 1 }]
    ),
    songOfMana: createWeapon('songOfMana', "Song of Mana", "🎶", "Vertical column of damage.", 'vertical-zone', 'skullOmaniac', 'mannajja', '#b794f4',
        { damage: 20, amount: 1, cooldown: 1500, area: 1.0, speed: 0, duration: 1000, knockback: 3, pierce: 99 },
        [{ damage: 5 }, { duration: 500 }, { damage: 5 }, { duration: 500 }, { damage: 5 }, { duration: 500 }, { area: 0.2 }]
    ),
    shadowPinion: createWeapon('shadowPinion', "Shadow Pinion", "🌑", "Strikes behind, then launches.", 'projectile-facing', 'wings', 'valkyrieTurner', '#000',
        { damage: 15, amount: 1, cooldown: 1000, area: 1, speed: 20, duration: 500, knockback: 3, pierce: 1 },
        [{ amount: 1 }, { damage: 5 }, { duration: 200 }, { amount: 1 }, { damage: 5 }, { duration: 200 }, { pierce: 1 }]
    ),
    clockLancet: createWeapon('clockLancet', "Clock Lancet", "🕰️", "Freezes enemies.", 'projectile-facing', 'torronasBox', 'infiniteCorridor', '#87CEEB',
        { damage: 0, amount: 1, cooldown: 1000, area: 1, speed: 15, duration: 3000, knockback: 5, pierce: 99, freeze: true },
        [{ duration: 500 }, { cooldown: -100 }, { duration: 500 }, { cooldown: -100 }, { duration: 500 }, { cooldown: -100 }, { amount: 1 }]
    ),
    laurel: createWeapon('laurel', "Laurel", "🌿", "Shields the player.", 'orbit', 'torronasBox', 'crimsonShroud', '#32CD32',
        { damage: 0, amount: 1, cooldown: 5000, area: 1, speed: 0, duration: 500, knockback: 5, pierce: 99, shield: 10 },
        [{ cooldown: -500 }, { shield: 10 }, { cooldown: -500 }, { shield: 10 }, { cooldown: -500 }, { shield: 10 }, { cooldown: -500 }]
    ),
    ventoSacro: createWeapon('ventoSacro', "Vento Sacro", "🍃", "Continuous attack in front.", 'directional-melee', 'crown', 'fuwalafuwaloo', '#98FB98',
        { damage: 10, amount: 1, cooldown: 200, area: 1, speed: 0, duration: 100, knockback: 1, pierce: 99 },
        [{ damage: 5 }, { area: 0.1 }, { damage: 5 }, { area: 0.1 }, { damage: 5 }, { area: 0.1 }, { damage: 5 }]
    ),
    bone: createWeapon('bone', "Bone", "🦴", "Bounces off enemies.", 'projectile-nearest', 'none', null, '#fff',
        { damage: 20, amount: 1, cooldown: 1000, area: 1, speed: 12, duration: 2000, knockback: 2, pierce: 99, bounceCount: 3 },
        [{ damage: 5 }, { speed: 2 }, { damage: 5 }, { bounceCount: 1 }, { damage: 5 }, { speed: 2 }, { bounceCount: 1 }]
    ),
    cherryBomb: createWeapon('cherryBomb', "Cherry Bomb", "🍒", "Explodes on impact.", 'projectile-nearest', 'none', null, '#ff0000',
        { damage: 30, amount: 1, cooldown: 1200, area: 1, speed: 10, duration: 0, knockback: 3, pierce: 0, isExplosive: true },
        [{ damage: 10 }, { area: 0.2 }, { damage: 10 }, { area: 0.2 }, { damage: 10 }, { area: 0.2 }, { amount: 1 }]
    ),
    carrello: createWeapon('carrello', "Carréllo", "🛒", "Mine cart that bounces.", 'boomerang', 'none', null, '#8B4513',
        { damage: 40, amount: 1, cooldown: 1500, area: 1.5, speed: 8, duration: 3000, knockback: 5, pierce: 99, bounceCount: 2 },
        [{ damage: 10 }, { speed: 2 }, { damage: 10 }, { bounceCount: 1 }, { damage: 10 }, { speed: 2 }, { amount: 1 }]
    ),
    celestialDusting: createWeapon('celestialDusting', "Celestial Dusting", "🌸", "Fires flowers behind you.", 'projectile-facing', 'none', null, '#FFC0CB',
        { damage: 15, amount: 2, cooldown: 500, area: 1, speed: 5, duration: 1000, knockback: 1, pierce: 99, angleOffset: Math.PI },
        [{ damage: 5 }, { duration: 500 }, { damage: 5 }, { duration: 500 }, { damage: 5 }, { duration: 500 }, { amount: 1 }]
    ),
    laRobba: createWeapon('laRobba', "La Robba", "🛋️", "Furniture falls from sky.", 'projectile-upward', 'none', null, '#8B4513',
        { damage: 30, amount: 1, cooldown: 1000, area: 1.5, speed: 10, duration: 0, knockback: 5, pierce: 99, gravity: 10 },
        [{ damage: 10 }, { area: 0.2 }, { damage: 10 }, { area: 0.2 }, { damage: 10 }, { area: 0.2 }, { amount: 1 }]
    ),
    victorySword: createWeapon('victorySword', "Victory Sword", "⚔️", "Slash nearest enemies.", 'directional-melee', 'torronasBox', 'soleSolution', '#FFD700',
        { damage: 50, amount: 1, cooldown: 800, area: 1.5, speed: 0, duration: 300, knockback: 5, pierce: 99 },
        [{ damage: 10 }, { area: 0.1 }, { damage: 10 }, { area: 0.1 }, { damage: 10 }, { area: 0.1 }, { amount: 1 }]
    )
};

const EVOLVED_WEAPONS = {
    bloodyTear: { name: "Bloody Tear", emoji: "🩸", type: 'directional-melee', stats: { damage: 60, amount: 1, cooldown: 1000, area: 2.0, lifesteal: 0.2 } },
    holyWand: { name: "Holy Wand", emoji: "🪄", type: 'projectile-nearest', stats: { damage: 30, amount: 1, cooldown: 50, speed: 20 } },
    thousandEdge: { name: "Thousand Edge", emoji: "🔪", type: 'projectile-facing', stats: { damage: 20, amount: 1, cooldown: 30, speed: 25 } },
    deathSpiral: { name: "Death Spiral", emoji: "🪓", type: 'projectile-spiral', stats: { damage: 60, amount: 9, cooldown: 1000, area: 2.5 } },
    heavenSword: { name: "Heaven Sword", emoji: "⚔️", type: 'boomerang', stats: { damage: 50, amount: 1, cooldown: 800, area: 1.5, speed: 15 } },
    unholyVespers: { name: "Unholy Vespers", emoji: "📕", type: 'orbit', stats: { damage: 30, amount: 6, cooldown: 0, duration: 999999 } },
    hellfire: { name: "Hellfire", emoji: "☄️", type: 'projectile-nearest', stats: { damage: 80, amount: 1, cooldown: 1000, area: 2.0, pierce: 99 } },
    soulEater: { name: "Soul Eater", emoji: "🌑", type: 'aura', stats: { damage: 20, amount: 1, cooldown: 100, area: 3.0, lifesteal: 0.05 } },
    laBorra: { name: "La Borra", emoji: "💧", type: 'zone-attract', stats: { damage: 40, amount: 4, cooldown: 1500, area: 2.5 } },
    noFuture: { name: "NO FUTURE", emoji: "🌈", type: 'projectile-facing', stats: { damage: 40, amount: 2, cooldown: 800, bounceCount: 99, isExplosive: true } },
    thunderLoop: { name: "Thunder Loop", emoji: "⛈️", type: 'lightning', stats: { damage: 50, amount: 4, cooldown: 1000, area: 2 } },
    gorgeousMoon: { name: "Gorgeous Moon", emoji: "🌑", type: 'screen-wipe', stats: { damage: 9999, amount: 1, cooldown: 15000 } },
    phieraggi: { name: "Phieraggi", emoji: "🔫", type: 'projectile-spiral', stats: { damage: 30, amount: 8, cooldown: 200, speed: 20 } },
    viciousHunger: { name: "Vicious Hunger", emoji: "👀", type: 'boomerang', stats: { damage: 40, amount: 2, cooldown: 800, isExplosive: true } },
    mannajja: { name: "Mannajja", emoji: "🌌", type: 'vertical-zone', stats: { damage: 60, amount: 1, cooldown: 1000, area: 2.0 } },
    valkyrieTurner: { name: "Valkyrie Turner", emoji: "🔥", type: 'projectile-facing', stats: { damage: 50, amount: 1, cooldown: 500, speed: 30, area: 2 } },
    infiniteCorridor: { name: "Infinite Corridor", emoji: "❄️", type: 'projectile-beam', stats: { damage: 0, amount: 1, cooldown: 500, freeze: true, area: 3 } },
    crimsonShroud: { name: "Crimson Shroud", emoji: "🛡️", type: 'aura', stats: { damage: 100, amount: 1, cooldown: 100, shield: 50 } },
    fuwalafuwaloo: { name: "Fuwalafuwaloo", emoji: "👻", type: 'directional-melee', stats: { damage: 40, amount: 2, cooldown: 100, area: 2 } },
    soleSolution: { name: "Sole Solution", emoji: "🌌", type: 'aura', stats: { damage: 100, amount: 1, cooldown: 50, area: 3.0 } }
};
...
const PASSIVE_ITEMS = {
    ...
    stoneMask: {
        key: 'stoneMask', name: "Stone Mask", emoji: "🗿", description: "Earn 10% more coins.",
        maxLevel: 5,
        apply: (rat) => { rat.greed += 0.1; }
    },
    skullOmaniac: {
        key: 'skullOmaniac', name: "Skull O'Maniac", emoji: "💀", description: "Increases Enemy Speed, Health, Quantity, and Frequency by 10%.",
        maxLevel: 5,
        apply: (rat) => { rat.curse += 0.1; }
    },
    torronasBox: {
        key: 'torronasBox', name: "Torrona's Box", emoji: "📦", description: "Increases Area, Duration, Speed, Might. Level 9 grants Curse.",
        maxLevel: 9,
        apply: (rat) => {
            // Simplified: Just buffs stats globally.
            // In VS it gives 4% per level up to level 8, then Curse.
            // We'll give 3% per level.
            rat.damageMultiplier += 0.03;
            rat.attackRangeMultiplier += 0.03;
            // rat.projectileSpeed? We don't have global projectile speed multiplier property easily accessible on Rat yet, handled in weapon stats.
            // Just apply what we can.
            if (rat.passives.find(p => p.key === 'torronasBox').level === 9) {
                rat.curse += 1.0; // Massive curse at max level
            }
        }
    },
    tiragisu: {
        key: 'tiragisu', name: "Tiragisú", emoji: "🍮", description: "Grants 1 Revival. Revivals restore 50% Health.",
        maxLevel: 2,
        apply: (rat) => { rat.revivals += 1; }
    }
};
...
class Rat extends Entity {
    constructor(characterKey, startX = 0, startY = 0) {
        ...
        this.curse = 1.0;        // New Curse stat (Enemy Speed/Health/Quantity)
        this.greed = 1.0;        // New Greed stat (Currency multiplier)
        this.revivals = 0;       // New Revive stat (Revival count)
        this.growth = 1.0;       // New Growth stat (XP Multiplier alias)
        this.rerolls = 0;        // New Reroll stat
        this.skips = 0;          // New Skip stat
        this.banishes = 0;       // New Banish stat

        // Visuals
        this.accessories = [];   // Array of { type: 'hat', color: 'red' }

        // --- APPLY POWER UPS ---
        Object.keys(purchasedPowerUps).forEach(key => {
            const rank = purchasedPowerUps[key];
            const def = POWER_UP_STATS[key];
            if (rank > 0 && def) {
                const totalBoost = def.amount * rank;
                if (typeof this[def.stat] === 'number') {
                    // Assuming most are multipliers (like damageMultiplier) or additive (armor)
                    // We need to know if it is additive or multiplier.
                    // Based on initialStats:
                    // Multipliers: damageMultiplier, attackRangeMultiplier, xpMultiplier, greed, curse, fireRateReduction (actually reduction multiplier?), luck
                    // Additive: armor, maxHp (usually %), hpRegen, projectileCountBonus, revivals, skips, rerolls

                    if (['damageMultiplier', 'attackRangeMultiplier', 'xpMultiplier', 'greed', 'curse', 'luck'].includes(def.stat)) {
                        this[def.stat] += totalBoost; // Additive percentage to base 1.0 (e.g. 1.0 + 0.1 = 1.1)
                    } else if (def.stat === 'maxHp') {
                        // Max HP % boost
                        const boostAmount = Math.floor(this.maxHp * totalBoost);
                        this.maxHp += boostAmount;
                        this.currentHp += boostAmount;
                    } else if (def.stat === 'fireRateReduction') {
                        // Cooldown reduction (additive to reduction factor? No, usually reduction %.
                        // Current logic: 1.0 / fireRateReduction. So higher is faster?
                        // Let's see Brawler: 1.0. Raca: 1.1 (faster?).
                        // "Weapon cooldown reduces by 5%". So we want this number to go UP?
                        // "1.0 / this.fireRateReduction". If reduction is 1.1, interval is smaller? Yes.
                        // So we ADD to the rate.
                        this[def.stat] += totalBoost;
                    } else {
                        // Pure additive
                        this[def.stat] += totalBoost;
                    }
        ...
        // Set accessories
        if (CHARACTERS[characterKey].accessories) {
            this.accessories = CHARACTERS[characterKey].accessories;
        }
    ...
    gainCurrency(amount) {
        this.currency += Math.floor(amount * this.greed);
    }
    ...
    fireWeapon(game, weapon) {
        ...
        switch (weapon.type) {
            ...
            case 'lightning':
                // Strike random enemies on screen
                const visibleTargets = game.getVisibleEnemies();
                if (visibleTargets.length > 0) {
                    for(let i=0; i<stats.amount; i++) {
                        ...
                        if (target) {
                            ...
                            game.addExplosionParticles(10, target.x, target.y, '#63b3ed', 4);
                            // Visual lightning bolt
                            const bolt = new Projectile(target.x, target.y - 300, 5, '#63b3ed', 0, Math.PI/2, 0, 0, false, 0, 'lightningRod');
                            bolt.prevX = target.x; // Start point
                            bolt.prevY = target.y - 300; // Sky
                            bolt.x = target.x; // End point
                            bolt.y = target.y;
                            if (target.currentHp <= 0) game.handleEnemyDefeat(target);
                        }
                    }
                }
                break;
            ...
            case 'screen-wipe':
                // Pentagram logic
                // Chance to fail (erase items)? Maybe simplified: just kill everything.
                game.addExplosionParticles(100, this.x, this.y, '#b794f4', 10); // Big purple flash
                game.enemies.forEach(e => {
                    const dist = Math.hypot(e.x - this.x, e.y - this.y);
                    if (dist < 800) { // Visible range
                        e.currentHp = 0;
                        game.handleEnemyDefeat(e);
                    }
                });
                break;

            case 'boomerang':
                // Fires facing, then returns
                const bAngle = this.lastFacingRight ? 0 : Math.PI;
                for(let i=0; i<stats.amount; i++) {
                    const angle = bAngle + (i * 0.1) - (stats.amount * 0.05);
                    this.spawnProjectile(game, weapon, stats, angle, 0, true); // true for return
                }
                break;

            case 'vertical-zone':
                // Song of Mana: Stack of zones
                const stackHeight = 600 * stats.area;
                const count = 10;
                for(let i=0; i<count; i++) {
                    // Above
                    game.pickups.push(new ZoneEntity(this.x, this.y - (i * 60 * stats.area), 40 * stats.area, stats.damage, stats.duration, WEAPONS[weapon.key].iconColor, weapon.key));
                    // Below
                    game.pickups.push(new ZoneEntity(this.x, this.y + (i * 60 * stats.area), 40 * stats.area, stats.damage, stats.duration, WEAPONS[weapon.key].iconColor, weapon.key));
                }
                break;

            case 'projectile-rear':
                const rearAngle = this.lastFacingRight ? Math.PI : 0;
                for(let i=0; i<stats.amount; i++) {
                    this.spawnProjectile(game, weapon, stats, rearAngle + (Math.random()-0.5)*0.5);
                }
                break;

            case 'projectile-falling':
                 for(let i=0; i<stats.amount; i++) {
                     const spawnX = this.x + (Math.random() - 0.5) * (VIEWPORT_RADIUS * 1.5);
                     const spawnY = this.y - VIEWPORT_RADIUS; // Top
                     // Manual projectile spawn to override params
                     const p = new Projectile(spawnX, spawnY, 20 * stats.area, WEAPONS[weapon.key].iconColor, stats.speed, Math.PI/2, stats.damage, 2000, false, 0, weapon.key, 10);
            ...
            case 'projectile-bounce-explode':
            case 'projectile-bounce-horizontal':
            case 'projectile-diagonal':
                // Treat as projectile-facing-ish
                let fAngle = this.lastFacingRight ? 0 : Math.PI;
                if (weapon.type === 'projectile-diagonal') {
                    // 45 degrees
                    fAngle = this.lastFacingRight ? -Math.PI/4 : -Math.PI*0.75;
                    // Add secondary projectiles? Guns shoot 4. Just spread them.
                }

                const spread = (weapon.type === 'projectile-diagonal') ? 0.5 : 0.1;

                for(let i=0; i<stats.amount; i++) {
                    const angle = fAngle + (i * spread) - (stats.amount * spread / 2);
                    this.spawnProjectile(game, weapon, stats, angle);
                }
                break;
        }
    ...
    spawnProjectile(game, weapon, stats, angle, gravity = 0, returnToPlayer = false) {
    ...
    drawRatBody(context) {
        ...
        this.accessories.forEach(acc => {
            context.save();
            if (acc.type === 'beard') {
                context.fillStyle = acc.color || '#fff';
                context.beginPath();
                context.moveTo(bodyLength * 0.4, headRadius * 0.5);
                context.quadraticCurveTo(bodyLength * 0.4 + 5, headRadius * 1.5, bodyLength * 0.4 - 10, headRadius * 1.2);
                context.quadraticCurveTo(bodyLength * 0.4 - 5, headRadius * 0.8, bodyLength * 0.4, headRadius * 0.5);
                context.fill();
            } else if (acc.type === 'hat_wizard') {
                context.fillStyle = acc.color || '#3182ce';
                context.beginPath();
                // Base
                context.ellipse(bodyLength * 0.4, -headRadius * 0.8, headRadius * 1.2, headRadius * 0.3, 0, 0, Math.PI*2);
                context.fill();
                // Cone
                context.beginPath();
                context.moveTo(bodyLength * 0.4 - headRadius * 0.8, -headRadius * 0.8);
                context.lineTo(bodyLength * 0.4 + headRadius * 0.8, -headRadius * 0.8);
                context.lineTo(bodyLength * 0.3, -headRadius * 2.5); // Tip back a bit
                context.fill();
            } else if (acc.type === 'glasses') {
                context.strokeStyle = acc.color || '#000';
                context.lineWidth = 1;
                context.beginPath();
                context.arc(bodyLength * 0.4 + headRadius * 0.6, -headRadius * 0.3, headRadius * 0.25, 0, Math.PI*2);
                context.stroke();
                context.beginPath();
                context.moveTo(bodyLength * 0.4 + headRadius * 0.35, -headRadius * 0.3);
                context.lineTo(bodyLength * 0.4 + headRadius * 0.1, -headRadius * 0.35); // Bridge
                context.stroke();
            } else if (acc.type === 'bandana') {
                context.fillStyle = acc.color || '#f00';
                context.beginPath();
                context.arc(bodyLength * 0.4, 0, headRadius, -Math.PI*0.8, -Math.PI*0.2); // Top part
                context.fill();
                // Knot
                context.beginPath();
                context.arc(bodyLength * 0.4 - headRadius, -headRadius * 0.5, 3, 0, Math.PI*2);
                context.fill();
            } else if (acc.type === 'ribbon') { // Imelda style
                context.fillStyle = acc.color || '#e53e3e';
                context.beginPath();
                context.arc(bodyLength * 0.3 + headRadius * 0.2, -headRadius * 0.7, headRadius * 0.2, 0, Math.PI*2);
                context.fill();
            } else if (acc.type === 'mohawk') { // Krochi style
                context.fillStyle = acc.color || '#f00';
                context.beginPath();
                for(let k=0; k<5; k++) {
                    const x = bodyLength * 0.4 - headRadius + (k*4);
                    const y = -headRadius * 0.8;
                    context.moveTo(x, y);
                    context.lineTo(x+2, y-8);
                    context.lineTo(x+4, y);
                }
                context.fill();
            } else if (acc.type === 'hood') { // Porta/Clerici
                context.fillStyle = acc.color || '#4B0082';
                context.beginPath();
                context.arc(bodyLength * 0.4, 0, headRadius * 1.1, 0, Math.PI * 2); // Larger circle behind/over head
                context.fill();
            }
            context.restore();
        });
...
class Enemy extends Entity {
    constructor(x, y, radius = 10, waveFactor = 1, type = 'bat') {
        super(x, y, radius, null, 2.0);
        this.type = type;

        let baseHp = 20 + (waveFactor - 1) * 15;
        let healthMultiplier = 1.0;
        if (waveFactor >= 5) healthMultiplier = 1.0 + ((waveFactor - 5) * 0.1);

        this.maxHp = Math.floor(baseHp * healthMultiplier);
        this.currentHp = this.maxHp;
        this.damage = 5 + (waveFactor - 1) * 5;
        this.xpValue = 2 + (waveFactor - 1) * 2;
    ...
    update(game, player, deltaTime) {
        ...
        if (this.isSlowed) actualSpeed *= 0.5;
        ...
        if (this.type !== 'ghost' && distance > 0 && game && game.obstacles) {
            game.obstacles.forEach(o => {
                ...
                if (dist < this.radius + o.radius) hitObstacle = true;
            });
        }
    ...
    draw(context) {
        ...
        if (this.type === 'bat') {
            // Flapping wings
            const flap = Math.sin(this.animationFrame * 0.5) * 10;
            context.fillStyle = this.color;
            context.beginPath();
            context.arc(0, 0, this.radius * 0.8, 0, Math.PI*2); // Body
            context.fill();
            // Wings
            context.beginPath();
            context.moveTo(-5, 0);
            context.quadraticCurveTo(-15, -10 + flap, -25, 0);
            context.lineTo(-5, 5);
            context.fill();
            context.beginPath();
            context.moveTo(5, 0);
            ...
            context.beginPath();
            context.arc(0, -5, 6, 0, Math.PI*2); // Skull
            context.fill();
            context.strokeStyle = '#fff';
            context.lineWidth = 2;
            context.beginPath();
            context.moveTo(0, 0);
            context.lineTo(0, 10); // Spine
            context.moveTo(-5, 5);
            context.lineTo(5, 5); // Ribs
            context.stroke();
        } else if (this.type === 'ghost') {
            context.globalAlpha = 0.6;
            context.fillStyle = this.color;
            context.beginPath();
            context.arc(0, 0, this.radius, Math.PI, 0); // Top half
...
class Projectile extends Entity {
    constructor(x, y, radius, color, speed, angle, damage, maxTravelDistance, isExplosive = false, currentBounces = 0, weaponKey, gravity = 0, returnToPlayer = false) {
        super(x, y, radius, color, speed);
        this.angle = angle;
        this.dx = Math.cos(angle) * speed;
        this.dy = Math.sin(angle) * speed;
    ...
    update(deltaTime, player) {
        ...
        if (this.returnToPlayer) {
            if (!this.returning) {
                ...
                // Start returning after half max distance or time (simplified to time/dist)
                const dist = Math.hypot(this.x - this.startX, this.y - this.startY);
                if (dist > this.maxTravelDistance * 0.4) { // Turn around at 40% max range (makes ellipse)
                     this.returning = true;
                }
            } else {
                ...
                // Increase speed on return
                const returnSpeed = this.speed * 1.5;
                this.x += (pdx / distToPlayer) * returnSpeed * (deltaTime / 16.66);
                this.y += (pdy / distToPlayer) * returnSpeed * (deltaTime / 16.66);

                if (distToPlayer < 20) return true; // Caught by player (despawn)
            }
        ...
        // Allow returning projectiles to exceed max distance if they are far away (they are returning)
        if (this.returnToPlayer) return this.elapsedTime > 5000; // Hard timeout
...
class SpawnDirector {
    constructor(game) {
        this.game = game;
        this.phases = [
            { start: 0, end: 60, spawnInterval: 800, maxEnemies: 50, types: ['bat'] },
            { start: 60, end: 120, spawnInterval: 600, maxEnemies: 100, types: ['bat', 'skeleton'] },
            { start: 120, end: 180, spawnInterval: 500, maxEnemies: 150, types: ['skeleton', 'zombie'] },
            { start: 180, end: 300, spawnInterval: 400, maxEnemies: 200, types: ['bat', 'skeleton', 'zombie'] },
            { start: 300, end: 600, spawnInterval: 200, maxEnemies: 400, types: ['skeleton', 'ghost', 'zombie'] },
            { start: 600, end: 900, spawnInterval: 100, maxEnemies: 600, types: ['bat', 'skeleton', 'ghost', 'werewolf'] },
            { start: 900, end: 99999, spawnInterval: 50, maxEnemies: 1000, types: ['bat', 'skeleton', 'ghost', 'zombie', 'werewolf'] }
        ];
    ...
    update(deltaTime) {
        ...
        this.game.enemySpawnInterval = Math.max(50, currentPhase.spawnInterval / this.game.player.curse); // More frequent
        this.game.maxEnemiesThisWave = Math.floor(currentPhase.maxEnemies * this.game.player.curse); // More enemies
...
class Game {
    ...
    spawnEnemy() {
        ...
        const allowed = this.allowedEnemyTypes || ['bat'];
        const type = allowed[Math.floor(Math.random() * allowed.length)];
        const waveFactor = this.currentMinute + 1;

        // Use base Enemy class with type
        const enemy = new Enemy(spawnX, spawnY, 12, waveFactor, type);

        // Apply Curse
        if (this.player.curse > 1.0) {
            enemy.speed *= this.player.curse;
            enemy.maxHp = Math.floor(enemy.maxHp * this.player.curse);
    ...
    checkCollisions(deltaTime) {
        ...
        this.projectiles = this.projectiles.filter(p => {
            ...
            return !(hit || p.update(deltaTime, this.player));
        });
    ...
    getVisibleEnemies() {
        const viewportRadius = VIEWPORT_RADIUS || 500;
        return this.enemies.filter(e => {
            const dist = Math.hypot(e.x - this.player.x, e.y - this.player.y);
            return dist < viewportRadius;
        });
    }
    ...
    endGame() {
        if (this.player.revivals > 0) {
            this.player.revivals--;
            this.player.currentHp = Math.floor(this.player.maxHp * 0.5);
            this.player.damageFlashTimer = 60; // 1 second invulnerability visual
            // Clear enemies nearby?
            this.addExplosionParticles(50, this.player.x, this.player.y, '#fff', 5);
            // Push enemies away
            this.enemies.forEach(e => {
                const angle = Math.atan2(e.y - this.player.y, e.x - this.player.x);
        ...
        // SAVE GOLD
        totalGold += this.player.currency;
        saveGame();
        ...
        document.getElementById('messageText').innerHTML = `
            ...
            Gold Earned: <span class="text-yellow-400">${this.player.currency}</span><br>
        ...
        if (hud.weaponSlots) {
            ...
            for (let i = 0; i < this.player.maxWeapons; i++) {
                ...
                if (i < this.player.weapons.length) {
                    ...
                    const level = w.evolved ? 'MAX' : w.level;

                    if (w.evolved) {
                         slot.className = 'w-8 h-8 bg-gray-900/80 border border-purple-500 rounded flex items-center justify-center relative shadow-sm mt-2';
                    }

                    slot.innerHTML = `
                        <div class="text-lg filter drop-shadow-md cursor-default" title="${name}">${emoji}</div>
                        <div class="absolute bottom-0 right-0 bg-black/80 text-[8px] px-1 rounded-tl text-white font-bold select-none">${level}</div>
                    `;
                } else {
...
function showStartScreen() {
    ...
    characterOptionsDiv.innerHTML = ''; // Clear previous

    // Create Container for Grid + Info
    const container = document.createElement('div');
    container.className = 'flex flex-col md:flex-row gap-6 w-full max-w-5xl mx-auto';

    // 1. LEFT: Character Grid
    const gridDiv = document.createElement('div');
    gridDiv.className = 'char-select-grid flex-1 border-2 border-gray-600 p-2 bg-gray-900/80 overflow-y-auto max-h-[60vh]';

    // 2. RIGHT: Info Panel
    const infoDiv = document.createElement('div');
    infoDiv.className = 'char-select-info w-full md:w-1/3 bg-gray-800 border-2 border-gray-600 p-4 flex flex-col justify-between hidden';
    infoDiv.id = 'charInfoPanel';

    // Populate Grid
    Object.entries(CHARACTERS).forEach(([key, char]) => {
        const icon = document.createElement('div');
        icon.className = 'char-select-icon relative group';
        icon.onclick = () => selectCharacter(key, icon);

        // Generate Mini-Portrait using SpriteCache approach
        const canvasSize = 64;
        const c = document.createElement('canvas');
        c.width = canvasSize;
        c.height = canvasSize;
        ...
        const mockRat = new Rat(key, canvasSize/2, canvasSize/2 + 5);
        mockRat.radius = 12; // Adjust size for icon
        mockRat.draw(ctx); // Use existing draw logic

        icon.appendChild(c);

        // Hover Tooltip (Name only)
        const tooltip = document.createElement('div');
        tooltip.className = 'absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 bg-black text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition pointer-events-none whitespace-nowrap z-10';
    ...
    characterOptionsDiv.appendChild(container);

    // Initial selection (first one)
    const firstKey = Object.keys(CHARACTERS)[0];
    const firstIcon = gridDiv.firstChild;
    if (firstKey) selectCharacter(firstKey, firstIcon);
    ...
    shopBtn.className = 'fixed top-4 right-4 bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded border-2 border-yellow-800 shadow-lg z-50';
    shopBtn.textContent = `POWER UP (Gold: ${totalGold})`;
    shopBtn.onclick = showShop;
    characterOptionsDiv.appendChild(shopBtn);
}

function showShop() {
    characterOptionsDiv.innerHTML = ''; // Clear main screen

    // Header
    const header = document.createElement('div');
    header.className = 'flex justify-between items-center mb-4 px-4';
    header.innerHTML = `
        <h2 class="text-3xl font-bold text-yellow-400">Power Up Selection</h2>
        <div class="text-2xl text-yellow-200">Gold: <span id="shopGoldDisplay">${totalGold}</span></div>
    `;
    characterOptionsDiv.appendChild(header);

    // Grid
    const grid = document.createElement('div');
    grid.className = 'shop-grid';

    Object.entries(POWER_UP_STATS).forEach(([key, power]) => {
        const currentRank = purchasedPowerUps[key] || 0;
        const isMaxed = currentRank >= power.maxRank;

        // Calculate cost (standard: cost * (rank + 1) * 0.1 flat increase? No, typically simpler in VS clones for now)
        // Let's stick to flat cost increase: Base Cost * (1 + 0.1 * Rank)
        const cost = Math.floor(power.cost * (1 + 0.1 * currentRank));

        const item = document.createElement('div');
        item.className = `shop-item ${isMaxed ? 'purchased-max' : ''}`;

        // Pips visualization
        let pipsHtml = '<div class="flex mt-2">';
        for(let i=0; i<power.maxRank; i++) {
            pipsHtml += `<div class="rank-pip ${i < currentRank ? 'filled' : ''}"></div>`;
        }
        pipsHtml += '</div>';

        item.innerHTML = `
            <div class="text-lg font-bold text-white">${power.name}</div>
            <div class="text-xs text-gray-300 text-center mb-2 h-8">${power.description}</div>
            ${pipsHtml}
            <div class="mt-auto pt-2 w-full">
                ${isMaxed
                    ? `<div class="text-center text-green-400 font-bold">MAXED</div>`
                    : `<button class="w-full bg-yellow-600 hover:bg-yellow-700 text-white text-sm font-bold py-1 px-2 rounded border border-yellow-800" onclick="buyPowerUp('${key}')">
                        ${cost} G
                       </button>`
                }
            </div>
    ...
    characterOptionsDiv.appendChild(grid);

    // Footer buttons
    const footer = document.createElement('div');
    footer.className = 'flex justify-center gap-4 mt-6';
    footer.innerHTML = `
        <button class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-6 rounded border-2 border-red-800" onclick="refundPowerUps()">REFUND POWER UPS</button>
        <button class="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-6 rounded border-2 border-gray-800" onclick="showStartScreen()">DONE</button>
    `;
    characterOptionsDiv.appendChild(footer);
}

// Global scope for onclick handlers (since removed type=module)
window.buyPowerUp = function(key) {
    const power = POWER_UP_STATS[key];
    const currentRank = purchasedPowerUps[key] || 0;

    if (currentRank >= power.maxRank) return;

    const cost = Math.floor(power.cost * (1 + 0.1 * currentRank));

    if (totalGold >= cost) {
        totalGold -= cost;
        purchasedPowerUps[key] = currentRank + 1;
        saveGame();
        showShop(); // Redraw
    }
};

window.refundPowerUps = function() {
    // Refund logic: Calculate total spent?
    // For simplicity, we just clear and give back a cheat amount or track spent?
    // Better to track spent. But I didn't save spent.
    // I will simply reset keys and NOT refund for this iteration (hardcore) or just give back based on current ranks calculation.
    ...
    Object.keys(purchasedPowerUps).forEach(key => {
        ...
        for(let i=0; i<rank; i++) {
             refundAmount += Math.floor(power.cost * (1 + 0.1 * i));
        }
    });

    totalGold += refundAmount;
    purchasedPowerUps = {};
    saveGame();
    showShop();
};

function selectCharacter(key, iconElement) {
    selectedCharacterKey = key;
    const char = CHARACTERS[key];
    const weaponDef = WEAPONS[char.starterWeapon];

    // Update visuals
    document.querySelectorAll('.char-select-icon').forEach(el => el.classList.remove('selected'));
    if (iconElement) iconElement.classList.add('selected');

    // Update Info Panel
    const infoPanel = document.getElementById('charInfoPanel');
    if (infoPanel) {
        infoPanel.classList.remove('hidden');
        infoPanel.innerHTML = `
            <div>
                <h2 class="text-2xl font-bold text-yellow-400 mb-1">${char.name}</h2>
                <div class="text-sm text-gray-400 mb-4 italic">"${char.description}"</div>

                <div class="bg-gray-900 p-2 rounded mb-4 border border-gray-700">
                    <div class="stat-row"><span>Max Health</span> <span class="text-green-400">${char.initialStats.maxHp}</span></div>
                    <div class="stat-row"><span>Might</span> <span class="text-red-400">+${Math.round((char.initialStats.damageMultiplier-1)*100)}%</span></div>
                    <div class="stat-row"><span>Speed</span> <span class="text-blue-400">${char.initialStats.baseSpeed}</span></div>
                    <div class="stat-row"><span>Recovery</span> <span class="text-pink-400">${char.initialStats.hpRegen}</span></div>
                    <div class="stat-row"><span>Area</span> <span class="text-purple-400">+${Math.round((char.initialStats.attackRangeMultiplier-1)*100)}%</span></div>
                    <div class="stat-row"><span>Cooldown</span> <span class="text-yellow-400">-${Math.round((char.initialStats.fireRateReduction-1)*100)}%</span></div>
                    ${char.initialStats.projectileCountBonus > 0 ? `<div class="stat-row"><span>Amount</span> <span class="text-orange-400">+${char.initialStats.projectileCountBonus}</span></div>` : ''}
                    ${char.initialStats.revivals > 0 ? `<div class="stat-row"><span>Revivals</span> <span class="text-white">${char.initialStats.revivals}</span></div>` : ''}
                </div>

                <div class="flex items-center gap-3 mb-4">
                    <div class="text-3xl bg-gray-700 w-12 h-12 flex items-center justify-center rounded border border-gray-500" style="color: ${weaponDef.iconColor}">
                        ${weaponDef.emoji}
                    </div>
                    <div>
                        <div class="text-xs text-gray-400">Starting Weapon</div>
                        <div class="font-bold text-white">${weaponDef.name}</div>
                    </div>
                </div>
            </div>

            <button id="startGameBtn" class="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-4 rounded transition shadow-lg border-2 border-red-800 text-xl mt-auto">
                START
            </button>
        `;

        document.getElementById('startGameBtn').onclick = () => startGame(key);
    }
}
