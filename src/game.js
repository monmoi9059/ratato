...
class Game {
    ...
    updateHUD() {
        ...
        if (this.player.buffs.explosiveBullets > 0) {
            buffText += `💥 E-Rounds: ${Math.ceil(this.player.buffs.explosiveBullets / 1000)}s | `;
        }
        if (this.player.buffs.iceAura > 0) {
            buffText += `❄️ Frozen: ${Math.ceil(this.player.buffs.iceAura / 1000)}s | `;
        }
        if (this.player.buffs.extremeSpeed > 0) {
            buffText += `💨 TURBO: ${Math.ceil(this.player.buffs.extremeSpeed / 1000)}s | `;
        }
